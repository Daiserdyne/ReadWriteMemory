...
public static class RwMemoryHelper
{
    ...
    /// <summary>
    /// Gives you a <see cref="External.RwMemory"/> instance which you have created
    /// before with the <see cref="CreateAndGetSingletonInstance(string)"/> function.
    /// </summary>
    ...
    public static RwMemory RwMemory =>
        _memory ?? throw new NullReferenceException("A RwMemory instance has to be created before you can get it. " +
                                                    $"Use the {nameof(CreateAndGetSingletonInstance)} method to " +
                                                    $"creat a instance, then you can use this property.");

    /// <summary>
    /// Gives you a thread-safe singleton instance of the <see cref="External.RwMemory"/> object.
    /// </summary>
    ...
    /// <summary>
    /// <para>Returns a Dictionary of all classes which have implemented the <seealso cref="IMemoryTrainer"/>
    /// interface in your entry assembly.</para>
    /// The key is the <see cref="IMemoryTrainer.TrainerName"/> and the value the instantiated Trainer.
    /// </summary>
    /// <returns>A dictionary of all implemented trainers. If no trainers are found, this will return a
    /// <c>empty</c> dictionary.</returns>
